...
public class DocumentSource
{
    ...
    private readonly MongoDBDatabaseWrapper _database;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public DocumentSource(IEntityType entityType, MongoDBDatabaseWrapper database)
    {
    ...
    public virtual string GetId(IUpdateEntry entry)
        => _idProperty is null
            ? throw new InvalidOperationException(MongoDBStrings.NoIdProperty(_entityType.DisplayName()))
