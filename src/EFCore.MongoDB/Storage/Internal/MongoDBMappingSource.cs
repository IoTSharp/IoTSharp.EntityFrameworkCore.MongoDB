...
/// </summary>
public class MongoDBMappingSource : TypeMappingSource
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public MongoDBMappingSource(TypeMappingSourceDependencies dependencies)
        : base(dependencies)
    {
        _clrTypeMappings
    ...
    private static CoreTypeMapping? FindPrimitiveMapping(in TypeMappingInfo mappingInfo)
    {
        ...
        {
            return new MongoDBTypeMapping(clrType);
        }
    ...
    private static CoreTypeMapping? FindCollectionMapping(in TypeMappingInfo mappingInfo)
    {
        ...
        if (clrType.IsArray)
        {
            ...
            return elementMapping == null
                ? null
                : new MongoDBTypeMapping(clrType, CreateArrayComparer(elementMapping, elementType));
        }
            ...
            && !clrType.IsGenericTypeDefinition)
        {
                ...
                || genericTypeDefinition == typeof(IReadOnlyList<>))
            {
                ...
                return elementMapping == null
                    ? null
                    : new MongoDBTypeMapping(clrType, CreateListComparer(elementMapping, elementType, clrType));
            }
                ...
                || genericTypeDefinition == typeof(IReadOnlyDictionary<,>))
            {
                ...
                return elementMapping == null
                    ? null
                    : new MongoDBTypeMapping(clrType, CreateStringDictionaryComparer(elementMapping, elementType, clrType));
            }
        }
