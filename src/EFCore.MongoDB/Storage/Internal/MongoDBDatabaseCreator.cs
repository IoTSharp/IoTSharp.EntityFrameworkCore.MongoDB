...
public class MongoDBDatabaseCreator : IDatabaseCreator
{
    ...
    public virtual bool EnsureCreated()
    {
        ...
        var created = _MongoDBClient.CreateDatabaseIfNotExists(model.GetThroughput());

        foreach (var container in GetContainersToCreate(model))
        {
            created |= _MongoDBClient.CreateContainerIfNotExists(container);
        }
    ...
    {
        var model = _designTimeModel.Model;
        var created = await _MongoDBClient.CreateDatabaseIfNotExistsAsync(model.GetThroughput(), cancellationToken)
            .ConfigureAwait(false);

        foreach (var container in GetContainersToCreate(model))
        {
            created |= await _MongoDBClient.CreateContainerIfNotExistsAsync(container, cancellationToken)
                .ConfigureAwait(false);
        }
    ...
    public virtual Task<bool> EnsureDeletedAsync(CancellationToken cancellationToken = default)
        => _MongoDBClient.DeleteDatabaseAsync(cancellationToken);
    ...
    public virtual bool CanConnect()
        => throw new NotSupportedException(MongoDBStrings.CanConnectNotSupported);
