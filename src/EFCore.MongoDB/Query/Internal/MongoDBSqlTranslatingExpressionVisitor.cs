...
/// </summary>
public class MongoDBSqlTranslatingExpressionVisitor : ExpressionVisitor
{
    ...
    private static readonly MethodInfo ParameterValueExtractorMethod =
        typeof(MongoDBSqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterValueExtractor));

    private static readonly MethodInfo ParameterListValueExtractorMethod =
        typeof(MongoDBSqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterListValueExtractor));
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public MongoDBSqlTranslatingExpressionVisitor(
...
#pragma warning restore IDE0066 // Convert switch statement to expression
    ...
    private sealed class SqlTypeMappingVerifyingExpressionVisitor : ExpressionVisitor
    {
        protected override Expression VisitExtension(Expression extensionExpression)
        {
            ...
            {
                throw new InvalidOperationException(MongoDBStrings.NullTypeMappingInSqlTree(sqlExpression.Print()));
            }
