...
/// </summary>
public class MongoDBStringMethodTranslator : IMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public MongoDBStringMethodTranslator(ISqlExpressionFactory sqlExpressionFactory)
    {
        ...
        if (instance != null)
        {
            ...
            if (TrimStartMethodInfoWithoutArgs.Equals(method) == true
                || (TrimStartMethodInfoWithCharArrayArg.Equals(method)
                    // MongoDB DB LTRIM does not take arguments
            ...
            if (TrimEndMethodInfoWithoutArgs.Equals(method) == true
                || (TrimEndMethodInfoWithCharArrayArg.Equals(method)
                    // MongoDB DB RTRIM does not take arguments
                    && ((arguments[0] as SqlConstantExpression)?.Value as Array)?.Length == 0))
            {
            ...
            if (TrimMethodInfoWithoutArgs.Equals(method) == true
                || (TrimMethodInfoWithCharArrayArg.Equals(method)
                    // MongoDB DB TRIM does not take arguments
                    && ((arguments[0] as SqlConstantExpression)?.Value as Array)?.Length == 0))
            {
